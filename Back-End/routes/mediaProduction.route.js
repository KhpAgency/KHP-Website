...
app.post(
  ...
  async (req, res, next) => {
    ...
    if (client) {
      ...
      let mediaProductionExist = await mediaProductionModel.find({
        name: clientName,
      });
      ...
      if (mediaProductionExist.length > 0) {
        res.json({
          message:
            "Media Production project already exists for this client name",
        });
      } else {
...
app.delete("/deleteMediaProduction", async (req, res) => {
  if (
